...
int main(int argc, char *argv[]){
	...
	uint32_t Delay_Depth1 = 10000 + (delay_multiplier * 5000);
	uint32_t Delay_Depth2 = 5000 + (delay_multiplier * 5000);
	uint32_t Delay_Depth3 = 2500 + (delay_multiplier * 5000);
	...
	{printf("bcm2835_spi_begin failed. Are you running as root??\n"); return 1;}
   	...
   	bcm2835_pwm_set_mode(0, 1, 1);                 //channel 0, markspace mode, PWM enabled. 
		...
		output_signal=(input_signal + (Echo_Buffer1[DelayCounter1])+(Echo_Buffer2[DelayCounter2])+(Echo_Buffer3[DelayCounter3]))>>decay_value;
